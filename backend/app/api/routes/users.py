...
@router.post(
    "/", dependencies=[require_role(["admin", "manager"])], response_model=UserPublic
)
...
# Note: With Keycloak-only authentication, password updates are handled by Keycloak
...
@router.get("/{user_id}", response_model=UserPublic, dependencies=[require_role(["admin", "manager"])])
def read_user_by_id(
...
@router.delete("/{user_id}", dependencies=[require_role(["admin", "manager"])])
def delete_user(
